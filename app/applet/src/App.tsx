...
import { Settings, Save, Download, Upload, Monitor, Edit3, Type } from 'lucide-react';
import { DeviceConfig } from './types';

export default function App() {
  const { inputs, outputs, selectedInput, selectedOutput, setSelectedInput, setSelectedOutput, messages, sendMidi } = useWebMidi();
    ...
    <div className={\`flex h-screen w-full flex-col \${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}\`}>
      ...
      <div className={\`flex items-center justify-between p-4 \${isDarkMode ? 'bg-slate-800' : 'bg-white shadow-sm'}\`}>
        ...
        <div className="flex gap-2">
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-500">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* SIDEBAR */}
        <div className={\`w-64 p-4 border-r \${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-slate-100/50'}\`}>
          <div className="flex flex-col gap-2">
            ...
            <button onClick={() => setActiveTab('devices')} className={\`flex items-center gap-2 p-2 rounded \${activeTab === 'devices' ? 'bg-blue-600 text-white' : 'hover:bg-slate-700/50'}\`}>
              <Type size={18} /> Devices (Templates)
            </button>
          ...
          <div className="mt-8">
            <h3 className="text-sm font-semibold mb-2 opacity-70">Active Device</h3>
            {BUILT_IN_DEVICES.map(d => (
              <button 
                key={d.id}
                onClick={() => setActiveDevice(d)}
                className={\`w-full text-left p-2 rounded text-sm mb-1 \${activeDevice?.id === d.id ? 'bg-slate-700 text-white' : 'hover:bg-slate-700/30'}\`}
              >
                {d.icon} {d.name}
              </button>
            ))}
        ...
        <div className="flex-1 p-6 overflow-auto">
          {activeTab === 'editor' && activeDevice && (
            <div>
              <h2 className="text-2xl font-bold mb-4">{activeDevice.name} Editor</h2>
          ...
          {activeTab === 'monitor' && (
            <div className="h-full flex flex-col">
              <div className="flex justify-between items-center mb-4">
                ...
                <button className="px-3 py-1 bg-slate-700 hover:bg-slate-600 rounded text-sm text-white" onClick={() => sendMidi([])}>Clear</button>
              </div>
            ...
            <div className="max-w-4xl">
              ...
              <div className={\`p-6 border rounded-xl shadow-sm \${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}\`}>
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex gap-2">
                    <button className="flex items-center gap-2 px-3 py-1.5 bg-green-600 text-white text-sm font-medium rounded hover:bg-green-500"><Save size={16}/> Save to Storage</button>
                    <button className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 text-white text-sm font-medium rounded hover:bg-slate-600"><Download size={16}/> Export JSON</button>
                    <button className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 text-white text-sm font-medium rounded hover:bg-slate-600"><Upload size={16}/> Import JSON</button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <input type="text" defaultValue={activeDevice?.name} className={\`w-full p-2.5 rounded-lg border \${isDarkMode ? 'bg-slate-900 border-slate-700 focus:border-blue-500 outline-none' : 'bg-slate-50 border-slate-300'}\`} />
                  </div>
                  <div>
                    <label className="block text-sm font-bold mb-2 opacity-80">Manufacturer</label>
                    <input type="text" defaultValue={activeDevice?.manufacturer} className={\`w-full p-2.5 rounded-lg border \${isDarkMode ? 'bg-slate-900 border-slate-700 focus:border-blue-500 outline-none' : 'bg-slate-50 border-slate-300'}\`} />
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <textarea 
                    className={\`w-full h-64 p-4 rounded-lg border font-mono text-sm leading-relaxed \${isDarkMode ? 'bg-[#0A0C10] border-slate-700 text-blue-300 focus:border-blue-500 outline-none' : 'bg-slate-50 border-slate-300'}\`}
                    defaultValue={JSON.stringify(activeDevice?.controls, null, 2)}
                  />
